...
abstract contract RealAbstractTokenization is Context {
    ...
    //
    // transfer Divident/Redeem token to msg.sender@Interface 
...
// File: Contracts/20221212/RealAPPCopyrightTokenization.sol
...
/*
  ...
  ############################################
    ...
    Real Matter Technology Limited (c) 2022
      ...
      Copyright Asset Tokens in ERC1155 NFT
...
// Interface Contract to access the real contract
//
contract RealAppCopyrightTokenization is RealAbstractTokenization {
    ...
    // REMARK : the only-checking is balanceOf[From]
    //
    function holder_Receive_From_Licensee(
        address fromPool,
        //address to,
        uint256 redeemLicenseFee_decimal18) 
        public {

        adminTransferRedeemFromPool2Sender(
          fromPool,
          //address to,
          redeemLicenseFee_decimal18
        );  
    } 
    ...
    // from the Real Contract owner
    // only original NFT asset at assetId.ID_ASSET = 1
    function owner_Mint_More_CopyrightShare_AssetToken(
        address toHolder,
        uint256 moreSupply) 
        public {
    ...
    // mint additional Asset Edition on top of the first NFT
    // auto tokenId > 1
    function owner_Mint_New_Copyright_EditionToken(
        address editionOwner,
        string memory editionURI,
        uint256 editionSupply)
        ...
        string memory realText0 = "New edition of Copyright minted [Token ID , New total Supply]";
        uint256 realNumber1 = _editionId;
